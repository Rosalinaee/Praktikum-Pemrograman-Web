...
<div class="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" class="w-full mt-1 p-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300" required>
        </div>

        <div>
            <label for="description" class="block font-medium text-gray-700">Deskripsi</label>
            <textarea name="description" rows="4" class="w-full mt-1 p-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">{{ old('description') }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="number" name="price" value="{{ old('price') }}" class="w-full mt-1 p-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300" required>
        </div>

        <div>
            <label for="download_link" class="block font-medium text-gray-700">Link Download</label>
            <input type="url" name="download_link" value="{{ old('download_link') }}" class="w-full mt-1 p-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">
        </div>

        {{-- Upload File PDF / ZIP / Gambar Digital --}}
        <div>
        ...
        <div class="flex items-center gap-4 pt-4">
            ...
            <a href="{{ route('products.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md">
                Batal
            </a>
        </div>
    </form>
